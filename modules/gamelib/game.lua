...
function g_game.setViewerMode(enabled)
  G.isViewerMode = enabled
  if enabled then
    g_logger.info('[Game] ✅ VIEWER MODE ENABLED - all controls BLOCKED')
  else
    g_logger.info('[Game] ❌ VIEWER MODE DISABLED - controls restored')
  end
end

function g_game.isViewerMode()
  return G.isViewerMode or false
end
