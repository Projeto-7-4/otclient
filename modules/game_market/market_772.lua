...
function init()
  ...
  print('[Market] Parent widget:', parentWidget)
  
  -- Try to load UI with detailed error handling
  marketWindow = nil
  local loadError = nil
  
  xpcall(function()
    if parentWidget then
      print('[Market] Attempting g_ui.loadUI with parent...')
      marketWindow = g_ui.loadUI('market', parentWidget)
    else
      print('[Market] Attempting g_ui.loadUI without parent...')
      marketWindow = g_ui.loadUI('market')
    end
  end, function(err)
    loadError = debug.traceback(err, 2)
  end)
  
  if marketWindow then
    print('[Market] ✅ UI loaded successfully!')
    print('[Market] marketWindow =', marketWindow)
  else
    print('[Market] ❌ ERROR: Failed to load market.otui')
    print('[Market] Full error:')
    print(loadError or 'Unknown error - loadError is nil')
    
    -- Try alternate path
    print('[Market] Trying alternate path: /modules/game_market/market')
    xpcall(function()
      marketWindow = g_ui.loadUI('/modules/game_market/market', parentWidget)
    end, function(err)
      print('[Market] Alternate path also failed:', debug.traceback(err, 2))
    end)
    
    if not marketWindow then
      print('[Market] All loading attempts failed!')
      return
    end
  end
